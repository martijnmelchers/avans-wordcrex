...
class Header {
    ...
    public void addPlayButton(MatchOverviewController controller, Consumer<Game> onClick, Game game, String comment) {
        createButton(onClick, game, comment, null, null, 0);
        addInformation(controller, game);
    }
    ...
    // Has to be called when the button is created.
    private void addInformation(MatchOverviewController controller, Game game)
    {
    ...
    private void AddTextFlowToButton(Insets margin, ArrayList<InformationData> textToAdd)
    {
        ...
        for (Node node : button.getChildren())
        {
            if(node instanceof Region)
            {
                ...
                region.prefWidthProperty().bind(button.widthProperty().divide(3));
            }
        }
    }
